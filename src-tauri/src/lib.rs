...
fn spawn_engine(app: &AppHandle) -> Result<EngineProc, String> {
    ...
    if !script.exists() {
        return Err(format!("Engine worker missing at {}", script.display()));
    }
    let mut child = spawn_python(&script)?;
    let stdin = child.stdin.take().ok_or("engine stdin missing")?;
    let stdout = child.stdout.take().ok_or("engine stdout missing")?;
    Ok(EngineProc {
...
fn read_until_terminal(
    ...
    let mut stdout = proc.stdout.lock().map_err(|e| e.to_string())?;
...
fn trim_wav(src: String, dest: String, start: f32, end: f32) -> Result<TrimResult, String> {
    let mut reader = hound::WavReader::open(&src).map_err(|e| e.to_string())?;
